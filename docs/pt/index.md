...
`tucoop` é uma biblioteca Python para **Teoria de Jogos Cooperativos com Utilidade Transferível (TU)**.
Ela oferece:
 ...
 - Um gerador de **especificações de animação em JSON**, compatível com o pacote `tucoopjs`.
...
## Escopo e objetivos de design
 ...
 - Dependências pesadas opcionais:
  ...
  - `tucoop[lp]` habilita métodos baseados em programação linear via `SciPy` (least-core, nucleolus, balanceamento, conjunto de barganha, ...)
  - `tucoop[lp_alt]` motor alternativo para programação linear via `PuLP` 
  - `tucoop[fast]` habilita utilitários baseados em `NumPy` (kernel / prekernel)
  - `tucoop[viz]` habilita visualização simples para jogos com 2 ou 3 jogadores baseada em `Matplotlib`

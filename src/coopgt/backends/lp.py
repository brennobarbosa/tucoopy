...
- This module requires an LP solver backend at runtime.
  - Recommended: install SciPy with `pip install "tucoop[lp]"` (uses `scipy.optimize.linprog` / HiGHS).
  - Alternative: install PuLP with `pip install "tucoop[lp_alt]"` (uses CBC via PuLP).
- If neither SciPy nor PuLP is available, functions that require LP will raise a
  `MissingOptionalDependencyError` (from `tucoop.backends.optional_deps`).
"""
...
def linprog_solve(
    ...
    *,
    ...
    Warnings
    --------
    - When `backend="scipy"`, SciPy must be installed (`pip install "tucoop[lp]"`).
    - When `backend="pulp"`, PuLP must be installed (`pip install "tucoop[lp_alt]"`).

...
tucoop to expose LP-based explanations in analysis outputs.
...
- the fallback :class:`tucoop.backends.lp.LinprogResult` wrapper used by the PuLP backend.

Notes
-----
SciPy is an optional dependency in tucoop. If SciPy (or the configured LP backend)
is unavailable, higher-level helpers may raise an `ImportError` suggesting
installing extra dependencies (e.g. ``pip install "tucoop[lp]"``).

Examples
--------
>>> from tucoop.diagnostics.linprog_diagnostics import LinprogDiagnostics, explain_linprog
...
def build_lp_explanations(game: GameProtocol, *, tol: float = 1e-9, max_list: int = 256) -> dict[str, Any]:
    ...
    >>> from tucoop.diagnostics.linprog import build_lp_explanations
    ...
    out["balancedness_check"] = {
        ...
        "weights_meta": {"computed_by": "tucoop-py", **weights_meta},

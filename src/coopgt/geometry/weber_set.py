...
- :class:`WeberSet`, a set-valued object exposing a polyhedral representation for
  small ``n`` via :class:`tucoop.geometry.PolyhedralSet`.
...
>>> from tucoop.geometry.weber_set import marginal_vector, WeberSet
...
def marginal_vector(game: GameProtocol, order: Sequence[int]) -> list[float]:
    ...
    >>> from tucoop import Game
    >>> from tucoop.geometry import marginal_vector
    >>> g = Game.from_coalitions(n_players=2, values={0:0, 1:0, 2:0, 3:1})
    >>> marginal_vector(g, [0, 1])
    [0.0, 1.0]
...
def weber_marginal_vectors(
    ...
    --------
    >>> from tucoop import Game
    >>> from tucoop.geometry import weber_marginal_vectors
    >>> g = Game.from_coalitions(n_players=2, values={0:0, 1:0, 2:0, 3:1})
...
def weber_sample(
    ...
    *,
    ...
    --------
    >>> from tucoop import Game
    >>> from tucoop.geometry import weber_sample
    >>> g = Game.from_coalitions(n_players=3, values={
    ...     0:0, 1:0, 2:0, 4:0,
    ...     3:0, 5:0, 6:0,
    ...     7:1,
...
class WeberSet:
    ...
    reusing :class:`~tucoop.geometry.polyhedron.PolyhedralSet` helpers.

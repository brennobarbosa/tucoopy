...
>>> from tucoop.geometry import EpsilonCore
...
def epsilon_core_polyhedron(
    ...
    *,
    ...
    --------
    >>> from tucoop import Game
    >>> from tucoop.geometry.epsilon_core_set import epsilon_core_polyhedron
...
class EpsilonCore:
    ...
    def check(self, x: list[float], *, tol: float = DEFAULT_GEOMETRY_TOL, top_k: int = 8):
        ...
        Notes
        -----
        This delegates to :func:`tucoop.diagnostics.epsilon_core_diagnostics.epsilon_core_diagnostics`.
...
def least_core_polytope(
    ...
    *,
    ...
    Requires
    --------
    SciPy at runtime (install with: ``pip install "tucoop[lp]"``).

    Examples
    --------
    >>> from tucoop import Game
    >>> from tucoop.geometry.epsilon_core_set import least_core_polytope
    >>> g = Game.from_value_function(n_players=3, value_fn=lambda S: float(len(S)))

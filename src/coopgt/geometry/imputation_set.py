...
>>> from tucoop.geometry import ImputationSet
...
class PreImputationSet:
    ...
    def poly(self) -> PolyhedralSet:
        ...
        --------
        >>> from tucoop import Game
        >>> from tucoop.geometry import PreImputationSet
        >>> g = Game.from_coalitions(n_players=2, values={0: 0, 3: 1})
    ...
    def check(self, x: list[float], *, tol: float = DEFAULT_GEOMETRY_TOL):
        ...
        `tucoop.diagnostics.checks.check_allocation`.
...
class ImputationSet:
    ...
    def check(self, x: list[float], *, tol: float = DEFAULT_GEOMETRY_TOL):
        ...
        Delegates to `tucoop.diagnostics.imputation_diagnostics.imputation_diagnostics`.
